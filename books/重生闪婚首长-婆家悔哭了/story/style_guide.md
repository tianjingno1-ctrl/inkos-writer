...
- **定名时机**：正文写完、手改定稿、`audit` 通过后再定章名（写作期可用占位名 `第N章` 或 `chN-待定`）；**ch1 时间锚类标题除外**（如「嫁人第七天」可在开写前锁定）
- **定名步骤**：读本章正文 → 抓章末钩子或最大爽点 → 改 `# 第N章` 行 → 重命名 `chapters/000N_*.md` → 同步 `index.json` · `chapter_summaries.md` · `current_focus.md`
