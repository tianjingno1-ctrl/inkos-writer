...
function buildWorldMutatorSystemPrompt(language: "zh" | "en"): string {
  if (language === "en") {
    return [
      ...
      "An edge's fromId/toId may be the entity's id OR its name (label) — the system resolves either to the right entity, so never skip an edge just because you can't track ids. Reuse an existing entity's id/name when the same person/thing reappears; never re-create it under a new id (or the panel shows the same person twice).",
  ...
  return [
    ...
    "edges 的 fromId / toId 填实体的 id 或直接填实体名字（label）都可以，系统会自动对应到现有实体——绝不要因为记不住 id 就干脆不写边。复用当前状态里已存在实体的 id/名字：同一个人/物再次出现时更新它，绝不要换个新 id 重建（否则侧栏会出现重复的同一个人）。",
    ...
    "下面是一个完整范例，照此结构产出——每个 entity 必须带 id/type/label，实物线索这样进 entities（绝不能只写进 summary），人物关系这样进 edges：",
    `{"eventId":"evt-1","turn":1,"actionKind":"look","summary":"周野检查死者随身物，发现半张船票与一把仓库钥匙，并起了疑心。","entities":{"upsert":[{"id":"actor_zhouye","type":"actor","label":"周野","summary":"接手命案的巡捕。","status":"警觉","updatedEventId":"evt-1"},{"id":"evidence_half_ticket","type":"evidence","label":"半张船票","summary":"死者右手攥着，残留沪—甬航线与196_日期。","status":"seen","updatedEventId":"evt-1"},{"id":"item_brass_key","type":"item","label":"黄铜钥匙","summary":"柄上刻浦发仓库B-7。","status":"collected","updatedEventId":"evt-1"}]},"edges":{"upsert":[{"fromId":"周野","type":"怀疑","toId":"账房先生"},{"fromId":"actor_zhouye","type":"持有","toId":"item_brass_key"}]},"stateSlots":{"upsert":[{"id":"slot_deadline","kind":"timer","label":"结案倒计时","value":3,"updatedEventId":"evt-1"}]}}`,
  ].join("\n");
}
